        ...
        <AuthenticatedLayout>
            ...
            <div className="mx-auto w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <h1 className="text-3xl font-semibold text-zinc-900">Hostels</h1>
                        <p className="mt-2 text-sm text-zinc-500">
                            Manage boarding hostels, room capacity, and the bed inventory used for per-session allocations.
                        </p>
                    </div>
                    <div className="flex gap-3">
                        <Link href={route("hostel-allocations.index")} className="rounded bg-slate-600 px-4 py-2 text-white hover:bg-slate-700">
                            Hostel Allocations
                        </Link>
                        <Link href={route("hostels.create")} className="rounded bg-emerald-600 px-4 py-2 text-white hover:bg-emerald-700">
                            Add Hostel
                        </Link>
                    </div>
                </div>
                                ...
                                <Trow key={hostel.id}>
                                    ...
                                    <Tdata>
                                        <div className="flex items-center justify-center gap-6">
                                            <Link href={route("hostels.edit", hostel.id)} className="text-emerald-600 hover:underline">
                                                Edit
                                            </Link>
                                            <button onClick={() => handleDelete(hostel.id)} className="text-red-600 hover:underline">
                                                Delete
                                            </button>
                                        </div>
                                    </Tdata>
