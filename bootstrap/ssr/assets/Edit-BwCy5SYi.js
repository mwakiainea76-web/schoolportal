...
import "ziggy-js";
function EditCurriculumUnit({ curriculum_unit }) {
  const hasInitialized = useRef(false);
  const mapping = curriculum_unit?.curriculum_mapping;
  const cycleLabel = curriculum_unit?.curriculum?.name ?? mapping?.curriculum?.name ?? "";
  const courseLabel = mapping?.course?.name ? `${mapping.curriculum?.name} (${mapping.course.name})` : mapping?.curriculum?.name ?? "";
  ...
  useEffect(() => {
    ...
    setData({
      curriculum_id: curriculum_unit.curriculum_id ?? curriculum_unit.curriculum_mapping?.curriculum_id ?? "",
      curriculum_mapping_id: curriculum_unit.curriculum_mapping_id ?? "",
            ...
            SearchSelect,
            {
              ...
              onChange: (cycle) => {
                ...
                setData({
                  ...
                  curriculum_mapping_id: String(cycleId) === String(data.curriculum_id) ? data.curriculum_mapping_id : ""
                });
              },
        ...
        /* @__PURE__ */ jsxs("div", { children: [
          ...
          /* @__PURE__ */ jsx(
            SearchSelect,
            {
              ...
              placeholder: data.curriculum_id ? "Search course under cycle..." : "Select cycle first...",
              preloadOptions: true,
              minSearchLength: 3,
