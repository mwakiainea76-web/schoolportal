...
const DirectoryTable = ({
  ...
  const exportToCsv = () => {
    ...
    const headers = Array.from(table.querySelectorAll("thead th")).map((cell) => cell.textContent.replace(/\s+/g, " ").trim()).filter(Boolean);
    ...
    const rows = Array.from(table.querySelectorAll("tbody tr")).map(
      (row) => Array.from(row.querySelectorAll("td")).map((cell, index) => ({ index, value: cell.textContent })).filter(({ index }) => !headerIndexesToSkip.includes(index)).map(({ value }) => escapeCsvValue(value))
  ...
  return /* @__PURE__ */ jsxs("div", { className: " py-6 font-sans", children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md printable-table min-w-full", children: [
      /* @__PURE__ */ jsx("div", { className: "overflow-x-auto min-w-full", children: /* @__PURE__ */ jsx(
        "table",
        {
          ref: tableRef,
