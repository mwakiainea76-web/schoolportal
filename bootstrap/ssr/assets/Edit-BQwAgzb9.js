...
function EditStudent({ student, courseCurricula = [] }) {
  const [step, setStep] = useState(1);
  const [stepErrors, setStepErrors] = useState({});
  const [validating, setValidating] = useState(false);
  const hasProgramVersionMappings = courseCurricula.length > 0;
  const existingProgramVersionMappingId = student.program_enrollment?.program_version_mapping_id || student.programEnrollment?.program_version_mapping_id || student.program_enrollment?.id || student.programEnrollment?.id || "";
  ...
  return /* @__PURE__ */ jsxs(AuthenticatedLayout, { children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "mx-auto w-full", children: [
      ...
      /* @__PURE__ */ jsxs("div", { className: "rounded-xl pt-2", children: [
