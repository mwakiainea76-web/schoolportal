...
function CreateHod({
  ...
  useEffect(() => {
    const messages = Object.entries(errors).filter(
      ([key, value]) => Boolean(value) && (key.startsWith("sessions.") || key === "course_version_unit_ids" || key === "trainer_staff_id" || key === "lecture_room_id")
    ).map(([, value]) => value);
    if (messages.length) {
      alert(messages[0]);
    }
  ...
  const updateScopedFilters = (nextValues) => {
    const nextCourseVersionMappingId = nextValues.course_version_mapping_id ?? data.course_version_mapping_id;
    const nextModuleNumber = nextValues.module_number ?? data.module_number;
    router.get(
      route("academic.timetables.hod.create"),
      {
        course_version_mapping_id: nextCourseVersionMappingId || "",
          ...
          "form",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxs("div", { className: "space-y-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-5", children: [
                ...
                }) : /* @__PURE__ */ jsx("p", { className: "text-sm text-zinc-400", children: data.course_version_mapping_id && data.module_number ? "No unassigned curriculum units are available for this course and module." : "Choose a versioned course and module to load curriculum units." }) })
              ] }),
              /* @__PURE__ */ jsxs("div", { className: "grid gap-6 sm:grid-cols-2", children: [
                /* @__PURE__ */ jsxs("div", { children: [
