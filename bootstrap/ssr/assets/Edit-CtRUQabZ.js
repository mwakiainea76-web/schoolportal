...
function EditStudent({
  ...
  const existingProgramVersionMappingId = student.program_enrollment?.program_version_mapping_id || student.programEnrollment?.program_version_mapping_id || "";
  const existingProgramId = student.program_enrollment?.program_id || student.programEnrollment?.program_id || student.program_enrollment?.program_version_mapping?.program?.id || student.programEnrollment?.programVersionMapping?.program?.id || "";
  const existingExamBodyId = student.program_enrollment?.exam_body_id || student.programEnrollment?.exam_body_id || student.program_enrollment?.program_version_mapping?.program?.certification_level?.exam_body_id || student.programEnrollment?.programVersionMapping?.program?.certificationLevel?.exam_body_id || "";
  const existingProgramVersionId = student.program_enrollment?.program_version_mapping?.program_version_id || student.programEnrollment?.programVersionMapping?.program_version_id || student.program_enrollment?.program_version_mapping?.programVersion?.id || student.programEnrollment?.programVersionMapping?.programVersion?.id || "";
  const { data, setData, put, processing, errors } = useForm({
    ...
    program_version_id: existingProgramVersionId,
  ...
  const hasProgramVersionMappings = courseOptions.length > 0;
  const hasLockedProgramEnrollment = Boolean(existingProgramVersionId);
  const selectedProgramLabel = student.program_enrollment?.program?.display_name || student.programEnrollment?.program?.display_name || student.program_enrollment?.program?.name || student.programEnrollment?.program?.name || student.program_enrollment?.program_version_mapping?.program?.name || student.programEnrollment?.programVersionMapping?.program?.name || "";
  const selectedCurriculumLabel = student.program_enrollment?.program_version?.name || student.programEnrollment?.programVersion?.name || student.program_enrollment?.program_version_mapping?.program_version?.name || student.programEnrollment?.programVersionMapping?.programVersion?.name || "";
  const handleChange = (e) => setData(e.target.name, e.target.value);
  useEffect(() => {
    if (!data.program_version_id && existingProgramVersionId) {
      setData("program_version_id", existingProgramVersionId);
    }
  ...
  useEffect(() => {
    ...
    axios.get(route("students.program-curricula", data.program_id)).then((response) => {
      const curricula = response.data ?? [];
      setCourseOptions(curricula);
      const selectedCurriculumStillExists = curricula.some(
        (curriculum) => String(curriculum.id) === String(data.program_version_id)
      );
      if (curricula.length === 1) {
        ...
        setData({
          ...
          program_version_id: curriculum.id,
          course_curriculum_id: curriculum.program_version_mapping_id ?? ""
        });
        return;
      }
  ...
  }, [data.program_id, hasLockedProgramEnrollment]);
  const submit = (e) => {
    e.preventDefault();
    ...
    /* @__PURE__ */ jsxs("div", { className: "mx-auto w-full", children: [
      ...
      /* @__PURE__ */ jsx("div", { className: "rounded-xl pt-2", children: /* @__PURE__ */ jsx("div", { className: "overflow-visible rounded-xl border bg-white shadow-sm", children: /* @__PURE__ */ jsxs("form", { onSubmit: submit, className: "space-y-5 p-0", children: [
        ...
        /* @__PURE__ */ jsxs("section", { className: "overflow-visible rounded-lg", children: [
          ...
          /* @__PURE__ */ jsx("div", { className: "p-4", children: /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3", children: [
            data.program_id && !hasProgramVersionMappings && !loadingCourses && !hasLockedProgramEnrollment ? /* @__PURE__ */ jsx("div", { className: "rounded border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 md:col-span-2 xl:col-span-3", children: "No active curriculum versions are available for the selected course." }) : null,
            /* @__PURE__ */ jsxs("div", { children: [
            ...
            /* @__PURE__ */ jsxs("div", { children: [
              ...
              /* @__PURE__ */ jsx(
                SearchSelect,
                {
                  ...
                  disabled: hasLockedProgramEnrollment,
                  preloadOptions: true,
                  minSearchLength: 3,
            ...
            /* @__PURE__ */ jsxs("div", { children: [
              ...
              /* @__PURE__ */ jsx(
                SearchSelect,
                {
                  ...
                  disabled: hasLockedProgramEnrollment || !data.program_id || loadingCourses,
                  ...
                  placeholder: loadingCourses ? "Loading curricula..." : "Select curriculum...",
                  error: errors.program_version_id
                },
                `curriculum-${data.program_id || data.program_version_id}-${courseOptions.length}`
              ),
              hasLockedProgramEnrollment ? /* @__PURE__ */ jsx("p", { className: "mt-1 text-xs text-slate-500", children: "Enrollment curriculum is locked after admission." }) : data.program_id && !hasProgramVersionMappings && !loadingCourses ? /* @__PURE__ */ jsx("p", { className: "mt-1 text-xs text-amber-600", children: "No active curricula are assigned to this course yet." }) : null,
              /* @__PURE__ */ jsx(
                InputError,
                {
