...
import { c as collectMatchingMergedUnits, h as hasExactOccupiedSlot, S as STUDY_SLOTS } from "./shared-D6ugPtRo.js";
  ...
  const moduleOptions = data.curriculum_mapping_id ? [
    ...new Set(
      curriculum_units.filter(
        (unit) => unit.curriculum_mapping_id === data.curriculum_mapping_id
      ).map((unit) => String(unit.module_taught || ""))
    )
  ].filter(Boolean).sort((a, b) => Number(a) - Number(b)) : [];
  const filteredUnits = data.curriculum_mapping_id && data.module_number ? curriculum_units.filter(
    (unit) => unit.curriculum_mapping_id === data.curriculum_mapping_id && String(unit.module_taught || "") === String(data.module_number)
  ) : [];
      ...
      "form",
      {
        ...
        children: [
          /* @__PURE__ */ jsxs("div", { className: "rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm text-emerald-800", children: [
            ...
            ". Assigned curriculum units stay visible for reference, but they remain locked so they cannot be double-assigned."
          ] }),
          /* @__PURE__ */ jsx(
            "input",
            {
          ...
          /* @__PURE__ */ jsxs("div", { className: "space-y-4 rounded-2xl border border-zinc-200 bg-zinc-50 p-5", children: [
            /* @__PURE__ */ jsxs("div", { children: [
              ...
              /* @__PURE__ */ jsx("p", { className: "text-sm text-zinc-500", children: "Select one curriculum unit to plan its timetable. Already assigned units stay locked. A unit can still share an existing live class when the trainer, room, and every planned study slot match exactly." })
            ] }),
            /* @__PURE__ */ jsx(
              InputError,
              {
            ...
            /* @__PURE__ */ jsx("div", { className: "grid gap-3 sm:grid-cols-2 xl:grid-cols-3", children: filteredUnits.length ? filteredUnits.map((unit) => {
            ...
            }) : /* @__PURE__ */ jsx("p", { className: "text-sm text-zinc-400", children: data.curriculum_mapping_id && data.module_number ? "No curriculum units are connected to the selected course version mapping and module." : "Choose a course version mapping and module to load curriculum units." }) })
          ] }),
