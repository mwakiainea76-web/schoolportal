...
function Index({
  ...
  const trainerFiltersReady = is_hod || is_trainer ? Boolean(draftFilters.academic_session_id) : Boolean(draftFilters.academic_session_id);
  const adminLoadPathReady = Boolean(
    draftFilters.academic_session_id && (draftFilters.trainer_staff_id || draftFilters.curriculum_mapping_id && draftFilters.module_number)
  );
