...
import "ziggy-js";
function EditCourseVersionUnit({ curriculum_unit }) {
  const hasInitialized = useRef(false);
  const mapping = curriculum_unit?.course_version_mapping;
  const cycleLabel = curriculum_unit?.course_version?.name ?? mapping?.course_version?.name ?? "";
  const courseLabel = mapping?.course?.name ? `${mapping.course_version?.name} (${mapping.course.name})` : mapping?.course_version?.name ?? "";
  ...
  useEffect(() => {
    ...
    setData({
      course_version_id: curriculum_unit.course_version_id ?? curriculum_unit.course_version_mapping?.course_version_id ?? "",
      course_version_mapping_id: curriculum_unit.course_version_mapping_id ?? "",
            ...
            SearchSelect,
            {
              ...
              onChange: (cycle) => {
                ...
                setData({
                  ...
                  course_version_mapping_id: String(cycleId) === String(data.course_version_id) ? data.course_version_mapping_id : ""
                });
              },
        ...
        /* @__PURE__ */ jsxs("div", { children: [
          ...
          /* @__PURE__ */ jsx(
            SearchSelect,
            {
              ...
              placeholder: data.course_version_id ? "Search course under cycle..." : "Select cycle first...",
              preloadOptions: true,
              minSearchLength: 3,
              onChange: (item) => setData("course_version_mapping_id", item.id),
              error: errors.course_version_mapping_id,
              disabled: !data.course_version_id
            },
            data.course_version_id || "no-cycle"
          ),
          /* @__PURE__ */ jsx(
            InputError,
            {
...
export {
  EditCourseVersionUnit as default
};
