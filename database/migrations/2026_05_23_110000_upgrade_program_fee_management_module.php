...
return new class extends Migration
{
    ...
    private function createFeePlanAssignmentsTable(): void
    {
        ...
        Schema::create('fee_plan_assignments', function (Blueprint $table) {
            ...
            $table->foreign('program_version_id')
                ->references('id')
                ->on('program_versions')
                ->cascadeOnUpdate()
                ->restrictOnDelete();
        ...
        DB::statement('CREATE UNIQUE INDEX fee_plan_assignments_active_unique ON fee_plan_assignments (program_version_id, academic_year_id, session_id) WHERE status = \'active\' AND deleted_at IS NULL');
    }
};
