...
class FeeAssignmentFilter
{
    ...
    {
        return $query
            ->when($filters['academic_session_id'] ?? null, fn ($q, $id) => $q->where('academic_session_id', $id)
            )

            ->when($filters['course_curriculum_id'] ?? null, fn ($q, $id) => $q->where('course_curriculum_id', $id)
            )
